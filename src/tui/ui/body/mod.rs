...
pub fn draw_main_body(frame: &mut Frame, app: &mut App, area: Rect) {
    ...
    list::draw_entry_list(frame, app, main_chunks[0]);
...
fn draw_detail_panels(frame: &mut Frame, app: &mut App, area: Rect) {
    ...
    description::draw_description_panel(frame, app, detail_chunks[0]);
    script::draw_script_panel(frame, app, detail_chunks[1]);
}
